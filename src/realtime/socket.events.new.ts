...
const handleAgentConnection = (io: Server, socket: Socket, user: any) => {
  ...
  socket.on('create_conversation', async (data: { otherUserEmail: string; inboxId?: string }) => {
    try {
      ...
      socket.emit('conversation_created', { conversation, targetUser });
  ...
  socket.on('get_conversations', async () => {
    try {
      ...
      const conversations = await ConversationService.getInboxConversations(user.tenantId, user.userId, 'mine');
